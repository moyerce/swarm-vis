...
However, we decided against using a spline method since they may distort the actual path of an agent too much.
...
The choices of using Qt and OpenGL are motivated by wanting to make SwarmVis cross-platform compatible.
...
For each visualization, we discuss what they convey, how they are implemented, how they are enabled in the interface,
...
An important feature of SwarmVis is the making tracking individual agents effortless.
...
These gap filling agents are colored purple due to their slightly slower movement.
...
We also plan on adding the ability to change the dot representation of the agent to better display direction.
...
take agent data as input from SwarmVis and return a list of lines to be drawn.
